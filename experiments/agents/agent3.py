...
# ─────────────────────────────────────────────────────────────────────────────
# PIECE 2 — LLM CREDIBILITY CHECK (gpt-oss-120b)
...
def llm_credibility_check(title: str, content: str) -> float:
    ...
    Guards (checked before calling the model):
      < 100 chars → 0.0 neutral (can't judge, lean toward neutral)
      < 500 chars → 0.0 neutral (too thin to classify reliably)

    Fallbacks (when model response is unexpected):
      empty response → 0.0 neutral (safety filter, never penalize)
      unparseable    → 0.0 neutral
      Groq crash     → 0.0 neutral (never discard on API failure)
...
Verdict:"""
    ...
    try:
        ...
        if not raw or not raw.strip():
            print(f"  [llm_cred_check] empty response (safety filter) → 0.0 neutral")
            return 0.0
        ...
        print(f"  [llm_cred_check] unparseable {label[:30]!r} → 0.0 neutral")
        return 0.0

    except Exception as e:
        print(f"  [llm_cred_check] Groq failed: {e} → 0.0 neutral")
        return 0.0
...
# Uses gpt-oss-20b for contradiction check (simple binary task, 
# preserves 120b quota for credibility classification).
# ─────────────────────────────────────────────────────────────────────────────
...
def check_credibility(story: dict) -> dict:
    ...
    print(f"  [cred] regime={regime} | "
          f"src={src:.2f}×{w_src} + "
          f"llm={llm:.2f}×{w_llm} + "
          f"verify={verify_score:.2f}×{w_verify} = {score:.2f}")
